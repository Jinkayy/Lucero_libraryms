    ...
    <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800">
        <div class="flex h-full flex-col p-6">
                ...
                <form action="{{ route('categories.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid gap-4 md:grid-cols-3">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="Enter category name" required
            ...
            <div class="flex-1 overflow-auto">
                <h2 class="mb-4 text-lg font-semibold text-neutral-900 dark:text-neutral-100">Categories List</h2>
                        ...
                        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                            @forelse($categories as $category)
                                <tr class="transition-colors hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                                    ...
                                    <td class="px-4 py-3 text-center text-sm text-neutral-900 dark:text-neutral-100">{{ $category->name }}</td>
                                    <td class="px-4 py-3 text-center text-sm text-neutral-600 dark:text-neutral-400">{{ Str::limit($category->description, 50) ?? 'N/A' }}</td>
...
<script>
    function editCategory(id, name, description) {
        ...
        // Set the form action URL to the update route (e.g., /categories/1)
        const form = document.getElementById('editCategoryForm');
        form.action = `{{ url('categories') }}/${id}`; // Adjust route as needed
